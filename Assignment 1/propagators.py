...
def prop_FC(csp: CSP, newVar=None):
    ...
    for constraint in constraints:
        if constraint.get_n_unasgn() == 1:  # Exactly one unassigned variable
            ...
            # If no values remain in the variable's current domain, it's a dead-end
        ...
        for var in constraint.get_scope():
            for value in var.cur_domain():
                if not constraint.check_var_val(var, value):
                    ...
                    # If no values remain, return failure (dead-end)
